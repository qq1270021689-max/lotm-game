...
describe('v9到v10迁移与资源旁路', () => {
  ...
  it('v9凡人保持null；序列9旧配方变legacy_unverified且required=1并幂等', () => {
    ...
    expect(loadGame()).toMatchObject({ schemaVersion: 18, sequence8Progress: null });
  ...
  it('v9已序列8迁移completed且不降级', () => {
    ...
    const loaded = loadGame()!;
    expect(loaded).toMatchObject({ schemaVersion: 18, sequence: 8, sequence8Progress: { stage: 'completed' } });
  });

import { CLUE_DEFS, DOCK_SEQUENCE9_ACTIONS, LOCATIONS } from './data';
import { isLocationUnlocked } from './location-access';
...
function dockCase(state: GameState): CaseJournalEntry | null {
  ...
  if (!discovered) return null;
  const concluded = facts.some(fact => fact.clueId === 'dock_seq9_conclusion');
  const pathwayFact = facts.find(fact => DOCK_PATH_CLUES.has(fact.clueId));
  ...
  const stage = concluded ? 'concluded' : pathwayFact ? 'pathway_inquiry' : secularFact ? 'investigating' : 'rumor';
  const directions: string[] = [];
  ...
  if (concluded) {
    ...
    directions.push(atDocks ? '在当前码头整合本途径记录与已有世俗线索。' : '回到已经查明的码头，整合本途径记录。');
    directions.push('尝试综合追查；若现有事实仍不足，就磨练与本途径直接相关的能力，或从其他已解锁地点寻找旁证。');
  ...
  return {
    ...
    statusLabel: concluded ? '已形成可复核的可靠结论' : pathwayFact ? '本途径现场记录已经建立' : secularFact ? '世俗记录正在形成证据链' : '目前只有失踪传闻',
    facts,
    unlockedLocations: unlockedLocations(state, ['docks', 'canal']),
    currentQuestion: concluded ? '哪些事实应当保留，以便将来继续追查幕后原因？'
      : pathwayFact ? '本途径观察如何与公开记录交叉印证？'
